// Copyright (c) 2015 The FlexCoin developers
		      ...
		      if(!fInvalid)
		      {
			    ...
			    { 
			        ...
			        if(ExtractDestination(atxout.scriptPubKey, dest))
			        {
			    	        ...
			    	        std::string flexcoinAddress = CBitcoinAddress(dest).ToString();
				        CRichListDataItem richListDataItem;
				        richListDataItem.sAddress = flexcoinAddress;
				        if(richListData.mapRichList.find(flexcoinAddress) == richListData.mapRichList.end())
				            richListData.mapRichList.insert(make_pair(flexcoinAddress, richListDataItem));
					
				        richListData.mapRichList[flexcoinAddress].nBalance = richListData.mapRichList[flexcoinAddress].nBalance - atxout.nValue;
