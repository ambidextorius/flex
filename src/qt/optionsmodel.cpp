...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeFlexCoinAmount"))
        settings.setValue("nAnonymizeFlexCoinAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeFlexCoinAmount = settings.value("nAnonymizeFlexCoinAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeFlexCoinAmount"))
        SoftSetArg("-anonymizeflexcoinamount", settings.value("nAnonymizeFlexCoinAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeFlexCoinAmount:
            nAnonymizeFlexCoinAmount = value.toInt();
            settings.setValue("nAnonymizeFlexCoinAmount", nAnonymizeFlexCoinAmount);
            emit anonymizeFlexCoinAmountChanged(nAnonymizeFlexCoinAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
