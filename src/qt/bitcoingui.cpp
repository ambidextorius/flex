    ...
    nWeight(0)
{
...
#endif
    //setObjectName("flexcoin");
    //setStyleSheet("#flexcoin { background-color: qradialgradient(cx: -0.8, cy: 0, fx: -0.8, fy: 0, radius: 1.4, stop: 0 #efefef, stop: 1 #dedede);  }");
    ...
    centralStackedWidget->addWidget(waveNet);
    ...
    //labelPrice->setText("Bittrex: 0.00000000 BTC/FCX")
    labelPrice->setText("NO DATA AVAIL!");
...
{
    QUrl url("https://bittrex.com/api/v1.1/public/getmarketsummary?market=btc-flexcoin");
...
{ 
    ...
    while(it.hasNext())
    {
        ...
        {
	    lastPrice = last;
	    waveNet->lastPrice = last;
	    //labelPrice->setText("Bittrex: " + QString::number(last, 'f', 8) + " BTC/FCX");
        labelPrice->setText("NO INFO AVAIL!");
        }
...
void BitcoinGUI::createActions()
{
    ...
    waveNetAction = new QAction(QIcon(":/icons/bitcoin"), tr("S&ling Road"), this);
    waveNetAction->setToolTip(tr("Browse the Market."));
    waveNetAction->setCheckable(true);
    tabGroup->addAction(waveNetAction);

    buysPageAction = new QAction(QIcon(":/icons/bitcoin"), tr("&Buys"), this);
    buysPageAction->setToolTip(tr("Show my Wave Net Buys."));
    ...
    sellsPageAction->setToolTip(tr("Show my Wave Net Sells."));
    sellsPageAction->setCheckable(true);
    tabGroup->addAction(sellsPageAction);

    masternodeManagerAction = new QAction(QIcon(":/icons/bitcoin"), tr("&Equlibilirium"), this);
    masternodeManagerAction->setToolTip(tr("Show Equlibilirium Nodes status and configure your nodes."));
    masternodeManagerAction->setCheckable(true);
    tabGroup->addAction(masternodeManagerAction);

    richListPageAction = new QAction(QIcon(":/icons/bitcoin"), tr("&Rich List"), this);
    richListPageAction->setToolTip(tr("Show top FlexCoin balances."));
    richListPageAction->setCheckable(true);
    tabGroup->addAction(richListPageAction);

    messageAction = new QAction(QIcon(":/icons/edit"), tr("SonicSend"), this);
    ...
    connect(waveNetAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(waveNetAction, SIGNAL(triggered()), this, SLOT(gotoWaveNet()));
...
void BitcoinGUI::createToolBars()
{
    ...
    toolbar->addAction(waveNetAction);
...
void BitcoinGUI::setNumI2PConnections(int count)
{
    ...
    labelI2PConnections->setToolTip(tr("%n active connection(s) to I2P-FlexCoin network", "", count));
}
#endif
...
void BitcoinGUI::gotoWaveNet()
{
    waveNetAction->setChecked(true);
    centralStackedWidget->setCurrentWidget(waveNet);
