    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat FlexCoin and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002FCX per transaction on average.
            */
            if(r <= 20)
            {
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeFlexCoinAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fFlexCoinSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f FCX CENT - sessionTotalValue %d\n", fFlexCoinSubmitted, sessionTotalValue);
...
void CDarkSendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100FCX+1 ( bit on if present )
    // bit 1 - 10FCX+1
    // bit 2 - 1FCX+1
    // bit 3 - .1FCX+1
    // bit 3 - non-denom
...
bool CDarkSendSigner::VerifyMessage(CPubKey pubkey, vector<unsigned char>& vchSig, std::string strMessage, std::string& errorMessage)
{
    ...
    CPubKey pubkey2;
    if (!pubkey2.Recover(ss.GetHash(), vchSig)) {
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("flexcoin-darksend");
        ...
        if(c % (60*5)*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeFlexCoinAmount = (rand() % nLeftToAnon)+3;
                } else {
