...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #flexcoincoinTEST\r");
            Send(hSocket, "WHO #flexcoincoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #flexcoincoin\n");
            // randomly join #flexcoincoin00-#flexcoincoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #flexcoincoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #flexcoincoin%02d\r", channel_number).c_str());
        };
