...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <flexcoinaddress>\n"
            "Return information about <flexcoinaddress>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <flexcoinpubkey>\n"
            "Return information about <flexcoinpubkey>.");
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <flexcoinaddress> <signature> <message>\n"
    ...
    CPubKey pubkey;
    if (!pubkey.Recover(ss.GetHash(), vchSig))
        return false;
